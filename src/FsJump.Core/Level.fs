...
let tilesetDecoder(jsonElement: JsonElement) = decode {
  ...
  and! tileCount = Required.Property.get ("tilecount", Required.int) jsonElement

  and! tileHeight =
    Required.Property.get ("tileheight", Required.int) jsonElement

  and! tileWidth = Required.Property.get ("tilewidth", Required.int) jsonElement

  and! tiles = Optional.Property.array ("tiles", tilesetTileDecoder) jsonElement
...
let mapObjectDecoder(jsonElement: JsonElement) = decode {
  ...
  and! customProperties =
    Optional.Property.array ("properties", wellKnownObjPropsDecoder) jsonElement
    |> Result.map(Option.defaultValue Array.empty)
...
let parseTileLayer (layer: TileLayer) tileset =
